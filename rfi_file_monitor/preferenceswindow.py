...
from .preferences import Preference, BooleanPreference, ListPreference, DictPreference, StringPreference, Preferences
...
class PreferenceValueCellRenderer(Gtk.CellRenderer):
    ...
    def __init__(self, window: PreferencesWindow, settings: Dict[Preference, Any], list_store: Gtk.ListStore, *args, **kwargs):
...
class PreferencesWindow(Gtk.Window):
    def __init__(self, prefs: Preferences, *args, **kwargs):
        ...
        value_renderer = PreferenceValueCellRenderer(window=self, settings=self._prefs.settings, list_store=store)
    ...
    def _operation_toggled(self, renderer: Gtk.CellRendererToggle, path: str):
        ...
        if count_toggled == 1 and self._operations_store[path][1] is True:
            dialog = Gtk.MessageDialog(transient_for=self,
                ...
                buttons=Gtk.ButtonsType.CLOSE, text=f"At least 1 operation must be selected!",)
            dialog.run()
            dialog.destroy()
            return
        self._operations_store[path][1] = not self._operations_store[path][1]
        self._prefs.operations[self._operations_store[path][2]] = self._operations_store[path][1]
    ...
    def _engine_toggled(self, renderer: Gtk.CellRendererToggle, path: str):
        ...
        if count_toggled == 1 and self._engines_store[path][1] is True:
            dialog = Gtk.MessageDialog(transient_for=self,
                modal=True, destroy_with_parent=True,
                message_type=Gtk.MessageType.ERROR,
                buttons=Gtk.ButtonsType.CLOSE, text=f"At least 1 engine must be selected!",)
            dialog.run()
            dialog.destroy()
            return
        self._engines_store[path][1] = not self._engines_store[path][1]
        self._prefs.engines[self._engines_store[path][2]] = self._engines_store[path][1]
