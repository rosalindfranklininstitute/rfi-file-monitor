...
class QueueManager(WidgetParams, Gtk.Grid):
    ...
    def __init__(self, appwindow):
        ...
        self.attach(created_status_promotion_grid, 0, self.options_child_row_counter, 1, 1)
        ...
        created_status_promotion_checkbutton = self.register_widget(Gtk.CheckButton(label='Promote files from \'Created\' to \'Saved\' after',
                ...
                hexpand=False, vexpand=False), 'created_status_promotion_active', desensitized=True)
        ...
        created_status_promotion_spinbutton = self.register_widget(Gtk.SpinButton(
            ...
            hexpand=False, vexpand=False), 'created_status_promotion_delay', desensitized=True)
        ...
        self.attach(saved_status_promotion_grid, 0, self.options_child_row_counter, 1, 1)
        self.options_child_row_counter += 1
        ...
        saved_status_promotion_spinbutton = self.register_widget(Gtk.SpinButton(
            ...
            climb_rate=5,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=False, vexpand=False), 'saved_status_promotion_delay', desensitized=True)
    ...
    def _add_horizontal_separator(self):
        self.attach(Gtk.Separator(
            ...
            0, self.options_child_row_counter, 1, 1
        )
        self.options_child_row_counter+= 1
    ...
    def start(self):
        if self._running:
            raise AlreadyRunning('The queue manager is already running. It needs to be stopped before it may be restarted')

        self._running = True
        self._timeout_id = GLib.timeout_add_seconds(1, self._files_dict_timeout_cb, priority=GLib.PRIORITY_DEFAULT)
        self.notify('running')
            ...
            for _filename, _file in self._files_dict.items():
                ...
                if _file.status == FileStatus.CREATED:
                    ...
                    if (time() - _file.saved) > self.params.saved_status_promotion_delay:
                        ...
                        path = _file.row_reference.get_path()
                        self._appwindow._files_tree_model[path][2] = int(_file.status)
                    ...
                    for child in self._appwindow._files_tree_model[iter].iterchildren():
