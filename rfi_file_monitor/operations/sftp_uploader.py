...
class SftpUploaderOperation(Operation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        advanced_options_grid.attach(self._get_chmod_grid('file', '644'), 0, 0, 1, 1)
        advanced_options_grid.attach(self._get_chmod_grid('directory', '755'), 1, 0, 1, 1)
    ...
    def _get_chmod_grid(self, kind: str, default_octal: str):
        ...
        chmod_grid._checkbutton = self.register_widget(Gtk.CheckButton(
        ...
        chmod_grid._entry = self.register_widget(Gtk.Entry(
            editable=False, text=default_octal,
            halign=Gtk.Align.START, valign=Gtk.Align.CENTER,
            hexpand=False, vexpand=False,
        ), f'{kind}_chmod_octal')
        chmod_grid._entry_handler_id = chmod_grid._entry.connect('changed', self._chmod_grid_entry_changed_cb, chmod_grid)
        title_grid.attach(chmod_grid._entry, 1, 0, 1, 1)

        label = Gtk.Label(
            label='<b>Read</b>', use_markup=True,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=True, vexpand=False,
        )
        chmod_grid.attach(label, 1, 1, 1, 1)
        label = Gtk.Label(
            label='<b>Write</b>', use_markup=True,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=True, vexpand=False,
        )
        chmod_grid.attach(label, 2, 1, 1, 1)
        label = Gtk.Label(
            label='<b>Execute</b>', use_markup=True,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=True, vexpand=False,
        )
        ...
        label = Gtk.Label(
            label='<b>User</b>', use_markup=True,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=True, vexpand=False,
        )
        chmod_grid.attach(label, 0, 2, 1, 1)
        label = Gtk.Label(
            label='<b>Group</b>', use_markup=True,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=True, vexpand=False,
        )
        chmod_grid.attach(label, 0, 3, 1, 1)
        label = Gtk.Label(
            label='<b>Others</b>', use_markup=True,
            halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER,
            hexpand=True, vexpand=False,
        )
        ...
        for i in range(3):
            for j in range(3):
                ...
                check_button._handler_id = check_button.connect('clicked', self._chmod_grid_check_button_clicked_cb, chmod_grid)
        ...
        with paramiko.SSHClient() as client:
            ...
            with client.open_sftp() as sftp_client:
                try:
                    ...
                    if params.force_folder_creation:
                        ...
                        if params.directory_chmod_enabled:
                            sftp_client.chmod(params.destination, int(params.directory_chmod_octal, base=8))
                        folder_created = True
                    else:
                        raise
                ...
                with tempfile.NamedTemporaryFile(delete=False) as f:
                    ...
                    tmpfile = f.name
                file_destination = params.destination + '/' + os.path.basename(tmpfile)
                try:
                    ...
                    try:
                        if params.file_chmod_enabled:
                            sftp_client.chmod(file_destination, int(params.file_chmod_octal, base=8))
                    except:
                        # cleanup
    ...
    def _run(cls, file: File, params: Munch, operation_index: int, processed_dirs: List[str], processed_dirs_lock: RLock):
        try:
            with paramiko.SSHClient() as client:
                client.load_system_host_keys()
                client.set_missing_host_key_policy(AutoAddPolicy if params.auto_add_keys else RejectPolicy)
                ...
                with client.open_sftp() as sftp_client:
                    ...
                    with processed_dirs_lock:
                        if dirname not in processed_dirs:
                            if not isdir(sftp_client, dirname):
                                ...
                                if params.directory_chmod_enabled:
                                    sftp_client.chmod(dirname, int(params.directory_chmod_octal, base=8))
                    ...
                    if params.file_chmod_enabled:
                        sftp_client.chmod(rel_filename, int(params.file_chmod_octal, base=8))
                    remote_filename_full = sftp_client.normalize(rel_filename)
