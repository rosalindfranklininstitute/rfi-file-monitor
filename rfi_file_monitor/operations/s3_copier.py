...
class S3CopierOperation(S3UploaderOperation):
    ...
    def run(self, file: S3Object):
        ...
        object_acl_options = self._get_dict_acl_options(self.appwindow.preflight_check_metadata, 'object_acl_options', ALLOWED_OBJECT_ACL_OPTIONS)
        # see https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3.html#S3.Client.put_object_tagging
        object_tags = self._get_dict_tagset(self.appwindow.preflight_check_metadata, 'object_tags')
        ...
        try:
            ...
            if remote_etag == file.etag:
                # attach metadata
                self._attach_metadata(file, client_destination_options['endpoint_url'], file.key, self.params, self.index)
                raise SkippedOperation('File has been copied already to the destination bucket')
        ...
        try:
            s3_destination_client.copy(
                ...
                Callback=S3ProgressPercentage(file, file.filename, self.index, float(file.size)),
