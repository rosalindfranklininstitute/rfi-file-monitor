...
for _dest, _source in SOURCES:
    ...
    for _src in _source.glob('*.pango'):
        ...
        if not old_contents[0].strip().startswith('<span size="xx-large">'):
            raise Exception(f'Invalid file {_src.name}: must start with <span size="xx-large">')
        ...
        for _line in old_contents:
            ...
            if _line.startswith('<span') and _line.endswith('</span>'):
                ...
                if len(tag.attrib) > 1:
                    raise Exception('<span> tags can only contain the size attribute')
                new_contents.append(f'{SIZES[size]} {text}')
            elif '<span' in _line:
                raise Exception('<span> blocks are currently only supported for titles')
            elif _line == '<tt>':
        ...
        if _dest == '_operations':
            if supported_file_formats_found or not matches:
                raise Exception('Supported File Formats title found but no corresponding values')
            ...
            if exported_file_format_found or len(matches) != 1:
                raise Exception('Exported File Format title found but no corresponding value')
